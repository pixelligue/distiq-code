...
**distiq-code** — локальный прокси для Claude Code CLI. Автоматически направляет простые задачи на дешёвые модели и кэширует повторные запросы.

```
Claude Code → localhost → [Умный роутинг] → Anthropic API
```
...
### Сжатие tool definitions (вдохновлено ClaudeSlim)

Опциональное сжатие определений инструментов для экономии **~100-300 токенов на запрос** (10-15%):
...
Что сжимается:
- **Названия инструментов**: `"Bash"` → `"B"`, `"Read"` → `"R"`, `"Grep"` → `"S"`
- **Ключи параметров**: `"file_path"` → `"f"`, `"command"` → `"c"`, `"pattern"` → `"p"`
- **Длинные описания**: обрезаются до 100 символов

⚠️ **Экспериментально**: Claude Code может сломаться при обновлении. Если возникают ошибки — отключите эту опцию.

> **Примечание:** Экономия меньше, чем у ClaudeSlim (который заявляет 80% на tools), потому что Claude Code уже использует компактные схемы. Но каждый сэкономленный токен — это вклад в общую оптимизацию вместе с умным роутингом и кэшированием.
...
# Сжатие
COMPRESSION_ENABLED=true             # LLMLingua-2 сжатие промптов (требует [compression])
COMPRESSION_TARGET_TOKENS=500
TOOL_COMPRESSION_ENABLED=false       # Сжатие tool definitions (экспериментально)
